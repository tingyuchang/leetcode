	...
	for i := 0; i < len(nums); i++ {
		if len(subArray) == 0 || subArray[len(subArray)-1] < nums[i] {
			...
			for r >= l {
				...
				if mid >= 1 && (subArray[mid] == nums[i] || subArray[mid-1] < nums[i] && subArray[mid] > nums[i]) {
					replaceIndex = mid
					break
				}
...
func Candy(ratings []int) int {
	leftMax := make([]int, len(ratings))
	rightMax := make([]int, len(ratings))
	leftMax[0] = 1
	rightMax[len(rightMax)-1] = 1
	...
	for i := 0; i < len(leftMax); i++ {
		sum += __Daily_Prac.Max(leftMax[i], rightMax[i])
	}
