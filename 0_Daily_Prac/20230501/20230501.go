...
func LadderLength(beginWord string, endWord string, wordList []string) int {
	...
	vivistedStr := make(map[string]bool)

	for i := range wordList {
		vivistedStr[wordList[i]] = false
	}
	...
	for len(queue) > 0 {
		current := queue[0]
		queue = queue[1:]
		vivistedStr[current] = true

		for nextWord, vivisted := range vivistedStr {
			if !vivisted {
				count := 0

				for i := 0; i < len(nextWord); i++ {
					if nextWord[i] != current[i] {
						count++
					}
				...
				if count == 1 {
					...
					backQueue = append(backQueue, nextWord)
				}
