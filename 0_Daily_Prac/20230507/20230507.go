...
func Trap(height []int) int {
	...
	for i := 1; i < n; i++ {
		sum += __Daily_Prac.Min(leftMax[i], rightMax[i]) - height[i]
	}
...
func MinSubArrayLen(target int, nums []int) int {
	sum, length, l, r := 0, math.MaxInt, 0, 0
...
func BuildTree(preorder []int, inorder []int) *Tree.TreeNode {
	...
	index := 0
	return constructTree(preorder, inorder, &index, 0, len(preorder)-1, indexMap)
}

func constructTree(preoder, inorder []int, index *int, start, end int, indexMaps map[int]int) *Tree.TreeNode {
	if *index > len(preoder) {
		return nil
	}

	rootValue := preoder[*index]
	root := &Tree.TreeNode{Val: rootValue}
	pivot := indexMaps[rootValue]
	*index++

	if pivot-start > 0 {
		root.Left = constructTree(preoder, inorder, index, start, pivot-1, indexMaps)
	}

	if end-pivot > 0 {
		root.Right = constructTree(preoder, inorder, index, pivot+1, end, indexMaps)
	}
	return root
