...
func NumberOfArrays(s string, k int) int {
	...
	*/
	...
	return dfsNumberOfArrays(s, k, 0, memo)
}

func dfsNumberOfArrays(s string, k int, start int, memo []int) int {
	if memo[start] != 0 {
		return memo[start]
	}
	...
	for i := start; i < len(s); i++ {
		...
		count = (count + dfsNumberOfArrays(s, k, i+1, memo)) % __Daily_Prac.MOD
	}
...
func LadderLength(beginWord string, endWord string, wordList []string) int {
	...
	for len(queue) > 0 {
		current := queue[0]
		queue = queue[1:]
		wordsMap[current] = true

		for str, available := range wordsMap {
			if !available {
				count := 0
				...
				if count == 1 {
					...
					backQueue = append(backQueue, str)
				}
			}
		}
