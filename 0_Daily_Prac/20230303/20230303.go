...
func MinWindow(s, t string) string {
	l, r, start, distant, lengthOfT := 0, 0, 0, math.MaxInt, len(t)
	...
	if distant != math.MaxInt {
		return s[start : start+distant]
	}
...
func Combination(candidates []int, target int) [][]int {
	...
	combination(candidates, target, 0, 0, &res, &currentRes)
	return res
}

func combination(candidates []int, target, currentSum, currentIndex int, res *[][]int, currentRes *[]int) {
	...
	for i := currentIndex; i < len(candidates); i++ {
		...
		*currentRes = append(*currentRes, candidates[i])
		combination(candidates, target, currentSum, i, res, currentRes)
