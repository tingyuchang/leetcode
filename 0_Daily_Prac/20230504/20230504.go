...
func LadderLength(beginWord string, endWord string, wordList []string) int {
	...
	for len(queue) != 0 {
		current := queue[0]
		queue = queue[1:]
		visitedWord[current] = true

		for word, visited := range visitedWord {
			if !visited {
				count := 0
	...
	for i := 0; i < len(nums); i++ {
		if len(subArray) == 0 || subArray[len(subArray)-1] < nums[i] {
			...
			for r >= l {
				...
				if mid > 0 && (subArray[mid] == nums[i] || (subArray[mid-1] < nums[i] && nums[i] < subArray[mid])) {
					replaceIndex = mid
					break
				}
...
func JumpII(nums []int) int {
	maxDistance, currentEnd := nums[0], nums[0]
