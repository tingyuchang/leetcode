...
func BuildTree(preorder []int, inorder []int) *Tree.TreeNode {
	...
	index := 0
	return conductTreeFromPreOrder(preorder, inorder, 0, len(preorder)-1, &index, indexMap)
}

func conductTreeFromPreOrder(preorder, inorder []int, start, end int, index *int, indexMap map[int]int) *Tree.TreeNode {
	...
	*index += 1
	...
	if pivot-start > 0 {
		root.Left = conductTreeFromPreOrder(preorder, inorder, start, pivot-1, index, indexMap)
	}

	if end-pivot > 0 {
		root.Right = conductTreeFromPreOrder(preorder, inorder, pivot+1, end, index, indexMap)
	}
...
on it is less or equal to target. Since the answer may be too large, return it modulo 1e9 + 7.
...
func NumSubseq(nums []int, target int) int {
	...
	for i := 0; i < len(nums); i++ {
		...
		if right >= i {
			ans = (ans + power[right-i]) % __Daily_Prac.MOD
		}
	}
